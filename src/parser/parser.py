...
def save_to_csv(filename, users_info):
    fieldnames = [
        'id', 'first_name', 'last_name', 'bdate', 'city', 'country',
        'home_town', 'mobile_phone', 'home_phone', 'university_name',
        'relation', 'personal', 'connections', 'site',
        'friends_count', 'followers_count', 'activities', 'interests',
        'music', 'movies', 'tv', 'books', 'games', 'about', 'quotes',
        'career', 'military', 'occupation'
    ]
    ...
    with open(filename, 'a', newline='', encoding='utf-8-sig') as csvfile:
        ...
        for user in users_info:
            ...
            city = user.get('city', {}).get('title') if isinstance(user.get('city'), dict) else user.get('city', user.get('home_town', ''))
            writer.writerow({
                'id': user.get('id', ''),
